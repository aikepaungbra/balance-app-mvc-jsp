...
public class AccessHistorySearch {
	...
	public Predicate[] where(CriteriaBuilder cb, Root<AccessHistory> root) {
		...
		if(null != dateFrom) {
			var date = dateFrom.atStartOfDay().atZone(ZoneId.systemDefault()).toInstant();
			params.add(cb.greaterThanOrEqualTo(root.get(AccessHistory_.id).get(AccessHistoryPk_.ACCESS_AT), date));
		}
		
		if(null != dateTo) {
			var date = dateTo.plusDays(1).atStartOfDay().atZone(ZoneId.systemDefault()).toInstant();
			params.add(cb.lessThan(root.get(AccessHistory_.id).get(AccessHistoryPk_.ACCESS_AT), date));
		}
